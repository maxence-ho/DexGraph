...
InstructionWidth* dexCreateInstrWidthTable(void)
{
    ...
    instrWidth = (InstructionWidth *)malloc(sizeof(InstructionWidth) * kNumDalvikInstructions);
    if (instrWidth == NULL)
        return NULL;
...
InstructionFlags* dexCreateInstrFlagsTable(void)
{
    ...
    instrFlags = (InstructionFlags *)malloc(sizeof(InstructionFlags) * kNumDalvikInstructions);
    if (instrFlags == NULL)
        return NULL;

    for (i = 0; i < kNumDalvikInstructions; i++) {
        OpCode opc = (OpCode) i;
        InstructionFlags flags = (InstructionFlags)0;
        ...
        case OP_DIV_INT_LIT8:
        case OP_REM_INT_LIT8:
            flags = (InstructionFlags)(kInstrCanContinue | kInstrCanThrow);
            break;
        ...
        case OP_INVOKE_INTERFACE:
        case OP_INVOKE_INTERFACE_RANGE:
            flags = (InstructionFlags)(kInstrCanContinue | kInstrCanThrow | kInstrInvoke);
            break;
        ...
        case OP_GOTO_16:
        case OP_GOTO_32:
            flags = (InstructionFlags)(kInstrCanBranch | kInstrUnconditional);
            break;
        ...
        case OP_IF_GTZ:
        case OP_IF_LEZ:
            flags = (InstructionFlags)(kInstrCanBranch | kInstrCanContinue);
            break;

        /* switch statements; if value not in switch, it continues */
        case OP_PACKED_SWITCH:
        case OP_SPARSE_SWITCH:
            flags = (InstructionFlags)(kInstrCanSwitch | kInstrCanContinue);
            break;
...
InstructionFormat* dexCreateInstrFormatTable(void)
{
    ...
    instFmt = (InstructionFormat *)malloc(sizeof(InstructionFormat) * kNumDalvikInstructions);
    if (instFmt == NULL)
        return NULL;
...
int dexGetInstrOrTableWidthAbs(const InstructionWidth* widths, const u2* insns)
{
    ...
    if (*insns == kPackedSwitchSignature) {
        ...
        width = dexGetInstrWidthAbs(widths, (OpCode)INST_INST(insns[0]));
    }
    return width;
}
