...
static bool swapMap(CheckState* state, DexMapList* pMap)
{
    ...
    bool first = true;
    u4 lastOffset = 0;
	int dexMapSize =  sizeof(DexMapItem);

    SWAP_FIELD4(pMap->size);
    count = pMap->size;

    CHECK_LIST_SIZE(item, count, dexMapSize);
...
static bool verifyAnnotationsDirectoryIsForDef(const CheckState* state,
    ...
    const DexAnnotationsDirectoryItem* dir = (const DexAnnotationsDirectoryItem*)filePointer(state, offset);
...
static void* swapTriesAndCatches(const CheckState* state, DexCode* code) {
    ...
    u4* handlerOffs = (u4*)malloc(handlersSize); // list of valid handlerOff values
...
static void* intraVerifyStringDataItem(const CheckState* state, void* ptr) {
    const u1* fileEnd = (const u1 *)state->fileEnd;
    const u1* data = (const u1 *)ptr;
    bool okay = true;
    ...
    while (okay && count--) {
        ...
        switch (item->type) {
            ...
            case kDexTypeClassDefItem: {
                ...
                u4* definedClassBits = (u4 *)malloc(arraySize);
